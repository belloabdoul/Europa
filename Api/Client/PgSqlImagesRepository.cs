    ...
    ISimilarImagesRepository
{
        ...
        PerceptualHashAlgorithm perceptualHashAlgorithm, CancellationToken cancellationToken = default)
    {
        ...
        return new ImageInfos(Guid.Empty, tempHash.Memory);
    }
        ...
        PerceptualHashAlgorithm perceptualHashAlgorithm, CancellationToken cancellationToken = default)
    {
        ...
        {
            var tableName = ConvertNameToSnakeCaseLower(collectionName);
            // await using var bulkInsert = await connection.BeginBinaryImportAsync(
            //     $"COPY {tableName}({ImageIdColumn},{ImageHashColumn},{ImageSimilaritiesColumn}) FROM STDIN (FORMAT BINARY)",
            //     cancellationToken);
            ...
            command.Parameters.AddWithValue(ImageIdColumn, NpgsqlDbType.Bytea, group.FileHash);
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        matchingFiles.AddRange(
            JsonSerializer.Deserialize<List<Similarity>>(ref test)!.ToDictionary(similarity => similarity.DuplicateId,
